...
class RvizWidget(QWidget):
    ...
    def load_rviz_cfg(self):
        ...
        config = rviz.Config()
        self.cfg_path = str(Path(__file__).parent / "cfg/rviz.rviz")
    ...
    def save_rviz_cfg(self):
        ...
        rospy.loginfo(f"\033[1;32m保存rviz配置文件，配置文件路径为：{self.cfg_path}\033[0m")
...
if __name__ == "__main__":
    ...
    rviz_widget = RvizWidget(0, 0, 0, 0)
    rviz_widget.resize(1000, 1000)
    # start Qt's main event loop
    rviz_widget.show()
    app.exec_()
